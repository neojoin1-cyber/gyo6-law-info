...
특성화고 학생, 선생님, 학부모, 학교 구성원이 취업과 학교 현장에서 필요한 법률, 규정, 지침, 서식을 쉽게 찾고, 승인된 회원은 학생용/선생님용 익명 상담실에 상담글을 남길 수 있는 정보 제공 서비스를 만듭니다.

AI 법률정보 도구는 공개 질문창이 아니라 관리자 답변 작성용 근거 검색 도구로 운영합니다.
...
- 공개 자료실과 회원 전용 익명 상담실 화면 구성 완료
- 학생 상담실과 선생님 상담실 출입구 분리
- 관리자 전용 AI 근거 검색 도구 구성
...
로컬 개발 서버는 관리자 권한으로 AI 근거 검색 도구를 검증할 때 PC의 Ollama가 있으면 질문 해석 후보를 한 번 더 만들고 같은 근거 안에서 문장을 읽기 좋게 보강할 수 있습니다. 기본 모델은 한국어·다국어 분류 보조와 16GB VRAM 환경의 속도 균형을 고려해 `qwen3:4b-instruct`를 사용합니다.
...
3. `AUTH_REQUIRED=true` 운영 상태에서 법률정보 AI와 공식자료 검색 접근 제한 유지
...
운영 배포는 `AUTH_REQUIRED=true`입니다. D1 `MEMBER_DB`, `OWNER_EMAILS` 설정과 마이그레이션을 기준으로 상담글은 승인 회원만 등록하고, 법률정보 AI와 공식자료 검색은 `admin`, `owner` 관리자 권한에서만 사용할 수 있습니다.
