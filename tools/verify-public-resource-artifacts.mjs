...
assert(Array.isArray(index.resources) && index.resources.length >= 1000, "index: public resource count regressed below treasure-library baseline");
assert(index.stats?.searchOnly === 0, "index: search-only resources must not be published");
assert((index.stats?.byType?.guide || 0) >= 330, "index: practical guide count regressed below treasure-library baseline");
assert((index.stats?.byType?.form || 0) >= 650, "index: practical form count regressed below treasure-library baseline");
assert(index.resources.filter((resource) => resource.linkKind === "file").length >= 1000, "index: directly downloadable file count regressed below treasure-library baseline");
assert((index.stats?.embeddedFormCandidates || 0) >= 650, "index: embedded form extraction queue regressed below treasure-library baseline");
...
assert((resourcesByCategory.privacyRecords || 0) >= 60, "index: privacy/records resources regressed below treasure-library baseline");
assert((resourcesByCategory.staffLabor || 0) >= 60, "index: staff labor resources regressed below treasure-library baseline");
...
assert(index.resources.filter((resource) => /하이파이브|hifive\.go\.kr/i.test(`${resource.provider} ${resource.url} ${resource.title}`)).length >= 80, "index: HIFIVE vocational resources regressed below treasure-library baseline");
assert(index.resources.filter((resource) => /충청북도교육청|충북교육청|cbe\.go\.kr/i.test(`${resource.provider} ${resource.url} ${resource.title}`)).length >= 30, "index: CBE official safety/form resources regressed below treasure-library baseline");
