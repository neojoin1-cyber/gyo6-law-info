...
assert.match(firebaseConfig, /projectId:\s*"gyo6--ebook"/);
assert.match(firebaseConfig, /authDomain:\s*"gyo6--ebook\.firebaseapp\.com"/);
assert.match(firebaseConfig, /GYO6_FIREBASE_CONFIG_URL\s*=\s*""/);
assert.match(firebaseConfig, /GYO6_AUTH_REQUIRED\s*=\s*true/);
assert.match(indexSource, /경북직업교육연구회 법률정보/);
assert.doesNotMatch(indexSource, /설탕과소금/);
assert.match(indexSource, /firebase-config\.js\?v=20260613-ebook-auth/);
assert.match(indexSource, /styles\.css\?v=20260615-answer-clean-v1/);
assert.match(indexSource, /누구나 부담 없이 쓰는 특성화고 규정·지침 Q&amp;A로 전환합니다/);
assert.match(indexSource, /무료 법률정보 질문하기/);
assert.match(indexSource, /기본 답변은 OpenAI API와 공식자료 조회 API를 호출하지 않습니다/);
...
assert.match(indexSource, /href="\.\/\?tool=legal#legalTool" target="_blank"/);
...
assert.match(indexSource, /업무영역을 더 좁힐 때만 열기/);
assert.match(indexSource, /답변 보기/);
assert.doesNotMatch(indexSource, /href="\.\/\?tool=guide#guideQa" target="_blank"/);
assert.match(indexSource, /https:\/\/pf\.kakao\.com\/_TTANn\/chat/);
assert.match(indexSource, /카카오톡 챗봇/);
assert.match(indexSource, /카카오톡에서는 각자 본인의 채팅방으로 열립니다/);
assert.match(indexSource, /운영자의 개인 카카오톡 대화방이 열리는 방식은 아닙니다/);
assert.match(indexSource, /카카오톡 챗봇 열기/);
assert.match(indexSource, /auth\.js\?v=20260615-answer-clean-v1/);
assert.match(indexSource, /policy-knowledge-base\.js\?v=20260615-answer-clean-v1/);
assert.match(indexSource, /policy-engine\.js\?v=20260615-answer-clean-v1/);
assert.match(indexSource, /app\.js\?v=20260615-answer-clean-v1/);
...
assert.match(appSource, /const requestedTool = getRequestedToolParam\(\);\s*const isToolWindow = Boolean\(requestedTool\)/);
assert.match(appSource, /law-tool-legal/);
assert.match(appSource, /law-tool-guide/);
assert.match(appSource, /function hydrateFromUrl\(\) {\s*const requestedTool = getRequestedToolParam\(\);\s*if \(!requestedTool\)/s);
