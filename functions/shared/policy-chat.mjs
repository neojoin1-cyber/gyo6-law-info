...
function getDomainSpecificKakaoSummary(result = {}, policyResponse = {}, frame = {}) {
  ...
  if (frame.domainCode === "classManagementGuidance") {
    ...
    if (/수업|지시|따르지|불응|수업방해|훈육|조치|선도|생활지도/.test(question)) {
      return getCompactSentence(
        "반복적인 수업 지시불응은 교원의 학생생활지도에 관한 고시의 생활지도·수업방해 기준을 먼저 보고, 지도 기록·상담·보호자 안내를 남긴 뒤 선도·징계가 필요할 때 초·중등교육법 제18조와 시행령 제31조 및 학교생활규정을 최종 대조합니다.",
        260
      );
    }
    return getCompactSentence(
      "학급관리·학생생활지도 사안입니다. 교원의 학생생활지도에 관한 고시, 초·중등교육법상 선도·징계 절차, 학생 인권, 상담·보호자 안내, 기록 보존과 민원 전환 위험을 먼저 확인하고 학교생활규정은 세부 집행 기준으로 대조합니다.",
      260
    );
  }
