...
2026-07-09 기준 자동 수집기는 경북교육청의 학생생활, 학교회계, 통합자료실, 현장체험학습, 인사정보, 감염병 대응, 학교운영위원회 게시판을 직접 조회한다. 또한 교육부·하이파이브 교육자료실에서 직업계고 현장실습 공통 매뉴얼, 서식모음집, 산업안전 자료, 실습일지 안내, 고졸청년 지원정책 자료를 직접 수집한다. 목록 검색에서 상세 게시글 번호를 찾고, 상세 화면의 `RAONKUPLOAD.AddUploadedFile(...)`, `/upload/...`, `fileDownload.do`, `FileDown.do` 형태의 첨부파일을 원문 파일로 승격한다.

공개 자료실에는 검색 결과 링크가 아니라 실제 공식 첨부파일이나 법령 원문만 들어간다. 현재 기준 검증선은 공개 자료 80건 이상, 지침 35건 이상, 서식 12건 이상, 직접 파일 40건 이상이다. 이 기준을 통과하지 못하면 scheduled workflow가 실패한다.

직업계고 핵심 자료는 별도 보호선이 있다. 하이파이브 자료, 직업계고 현장실습 공통 매뉴얼, 편집 가능한 한글 서식·서식모음집이 빠지면 검증이 실패한다.
